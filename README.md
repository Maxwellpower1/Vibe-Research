...
| 🌊&nbsp;**期&#8288;权&#8288;/&#8288;期&#8288;货** | 顶栏 `/derivatives`（紧挨 A 股，旧 `/ovlab` 书签自动跳转）：**驾驶舱一屏** = 股指·商品·自选（三列一卡：主力合约码+价/涨跌/IVP/分时，自选列本机 `deriv.watch` 具体合约代码）· 商品板块 · 异动+到期（NEW 标记）· 隐波/IV百分位/IV溢价（点列头排序）· 涨跌榜 · 广度（涨跌分布+IVP 直方图）· 期限结构 · **T 型报价联动区**（行权价链：理论价=Black-76/IV 买卖/Delta/持仓，按到期月切换；点品种行切品种，点合约档位 → 右侧「期权日K（分钟聚合+标的隐波日线）/ 期权分时（零轴=昨结，叠合约隐波）」两卡联动出图）· 持仓排名摘要。内页签：K线（复刻 A股轻量图：自选合约列表 `deriv.kline.watch` + 分时/5日/日线，分时零轴=昨结，叠 ATM隐波/持仓量，右侧行情卡）/ 详情（全市场表·单品种·持仓历史·持仓排名·机构观点）/ T报价 / 异动资金流。数据全部走 **OpenVlab** 公开缓存（市场概览 / flow-alert / price-volatility-series），60s 轮询同一把钥匙，盘中按 TTL 刷新、**休市（盘后/午休/周末）冻结只读上一笔**，后端启动时补一次首屏；CTP 账户只留在「我的持仓」 |
