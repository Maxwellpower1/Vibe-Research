...
>
> A股 / 美股 / 期权期货、资讯雷达、个股数据、自选股、我的持仓、我的研报、研究记录。把数据和功能配齐，由**你自己的 AI** 驱动投资研究。
...
| 页面 | 包含的模块 / 能力 |
|---|---|
| 🇨🇳&nbsp;**A&#8288;股** | Tab：**每日复盘**（大盘 / 全球市场 / 自选 / 短线情绪 / 成交额 TOP20 / 板块资金 / AI 复盘）· **轻量图表**（自选列表 + 分时 / 5日 / 日K前复权，腾讯源，零 mootdx） |
...
| 🇺🇸&nbsp;**美&#8288;股** | 本地观察列表（ticker）· 东财快照行情 · **日 K + 成交量**（默认前复权 Yahoo，不可达回退新浪）。点列表即切图；只客观呈现，不推荐不预测 |
...
- **美股日 K**：`GET /api/global/us/kline?symbol=AAPL&num=180`（默认**前复权**：Yahoo `adjclose` 缩放 OHLC；Yahoo 不可达回退新浪不复权；缓存 5 分钟）；前端「美股」页 = 本地观察列表 + K 线/成交量图。
...
- **上游**：<https://github.com/simonlin1212/global-stock-data> —— 想要分钟线 / 技术指标 / 期权 / SEC 等全量端点，去这里看。
