...
| 页面 | 包含的模块 / 能力 |
|---|---|
| 🇨🇳&nbsp;**A&#8288;股** | 顶栏：**复盘** / **K线** / **详情** / **公告**。整站顶栏下方横向滚动行情条（全球指数 + 黄金/原油/BTC 等商品，与驾驶舱共用 5 秒报价中心）。复盘是一屏驾驶舱（可放大面板）：**全球关键指数** / **市场板块实时热点**（行业/概念，左领涨/右领跌，点板块在另半边看成分股）/ **实时热点 · 7×24**（财联社 / 新浪/见闻，行上产业链/宏观/政策标签 + NEW，新进条可自动滚到顶）/ **板块资金流向**（分钟累计蝴蝶图，点击筛主力净流入）/ **主力净流入排行** / **个股榜单** / **宏观观察**（标的：商品/NQ + 恒生/恒科/日韩 + 现期/基差）/ **涨跌分布 / 广度**（涨平跌条 + 8 档直方图 + 平均/中位，在宏观观察右侧）/ 涨跌停格 / 自选 / 龙虎·资金·8 条产业链。 |
...
| 🌊&nbsp;**期&#8288;权&#8288;/&#8288;期&#8288;货** | 顶栏 `/derivatives`（紧挨 A 股，旧 `/ovlab` 书签自动跳转）：**驾驶舱一屏** = 股指·商品·自选（三列一卡：主力合约码+价/涨跌/IVP/分时，自选列本机 `deriv.watch` 具体合约代码）· 商品板块 · 异动+到期（NEW 标记）· 隐波/IV百分位/IV溢价（点列头排序）· 涨跌榜 · 广度（涨跌分布+IVP 直方图）· 期限结构（选中品种远期曲线 今实线/昨虚线 + 跨品种近远月斜率排行，与 T 型报价联动）· **T 型报价联动区**（行权价链：理论价=Black-76/IV 买卖/Delta/持仓，按到期月切换；点品种行切品种，点合约档位 → 右侧「期权日K（分钟聚合+标的隐波日线）/ 期权分时（零轴=昨结，叠合约隐波）」两卡联动出图）· 持仓排名摘要。内页签：K线（复刻 A股轻量图：自选合约列表 `deriv.kline.watch` + 分时/5日/日线，分时零轴=昨结，叠 ATM隐波/持仓量，右侧行情卡）/ 详情（全市场表·单品种·持仓历史·持仓排名·机构观点）/ T报价 / 异动资金流。数据全部走 **OpenVlab** 公开缓存（市场概览 / flow-alert / price-volatility-series），60s 轮询同一把钥匙，盘中按 TTL 刷新、**休市（盘后/午休/周末）冻结只读上一笔**，后端启动时补一次首屏；CTP 账户只留在「我的持仓」 |
