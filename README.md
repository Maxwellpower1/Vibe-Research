...
| 页面 | 包含的模块 / 能力 |
|---|---|
| 🇨🇳&nbsp;**A&#8288;股** | 顶栏：**复盘** / **K线** / **详情** / **公告**。整站顶栏下方横向滚动行情条（全球指数 + 黄金/原油/BTC 等商品 + 美债 10Y·2Y，与驾驶舱共用 5 秒报价中心）。复盘是一屏驾驶舱（可放大面板）：**全球关键指数**（A/港/美/汇率，美元人民币有分时；A/港显示成交额，右侧上点位下涨跌幅）/ **市场板块实时热点**（行业/概念，右侧成分股可轮播）/ **涨跌分布 / 广度**（涨平跌条 + 8 档直方图 + 平均/中位）/ **板块资金流向**（分钟累计蝴蝶图，点击筛主力净流入）/ **主力净流入排行**（分时 + 净额/净占比）/ **个股榜单**（热门/涨跌，分时 + 主力资金 + 成交额）/ **大宗商品**（期货分时 + **生意社现期/基差表**） / 涨跌停格：**涨停梯队** / **跌停梯队**（首板·首跌到 5+）/ 自选（格内搜名称/代码加减，个股行星标一键加自选，行内行业标签）/ 龙虎·资金·8 条产业链（相关板块 + 问财刷新名单）。**实时热点 · 7×24 快讯**仍是右下角悬浮球（财联社 / 新浪/见闻） |
...
- **美股日 K**：`GET /api/global/us/kline?symbol=AAPL&num=180`（Yahoo，失败回退新浪，再回退 Stooq）；**港股日 K**：`GET /api/global/hk/kline?symbol=00700`（Yahoo query1/query2，403 回退腾讯 ifzq 前复权）。A 股日 K 腾讯/mootdx 空时回退 **Baostock**（可选包）。
