...
| 🌊&nbsp;**期&#8288;权&#8288;/&#8288;期&#8288;货** | 顶栏 `/derivatives`（紧挨 A 股，旧 `/ovlab` 书签自动跳转）：**驾驶舱一屏** = 行情观察（一张竖表：价/涨跌/隐波/IV分位色带/溢价/分时，点列头整表排序；卡内 tab 切自选 `deriv.watch`）· 临期期权日历（当月未过期月历，切月看远月，格子标交易所，点/悬停看标的）· 期限结构（品种可搜；选中品种：现值/涨幅表 + 远期曲线 今实线/昨虚线 + 同月持仓量柱 + 仓单最新/日变/近90日，在异动左侧）· 异动（成交/走势/连续可关 · 剩余天数 · 区间涨幅 · 区间成交量 · 本机阈值）· **T 型报价联动区**（品种可搜；品种旁大号标的最新/涨跌；全部行权价链：理论价=Black-76/IV 买卖/Delta/持仓，按到期月切换；默认 ATM 购出图，点品种行切品种，点合约档位切图；日K/分时同一张卡上下叠在右侧一小条，主看 T 表；日K=分钟聚合+标的隐波日线，分时=昨结零轴+合约隐波+成交量，X 轴按交易时段铺满不拉满）。内页签：K线（复刻 A股轻量图：自选合约列表 `deriv.kline.watch` + 分时/5日/日线，分时零轴=昨结，轴按交易时段，叠 ATM隐波/持仓量，右侧行情卡）。数据全部走 **OpenVlab** 公开缓存（市场概览 / flow-alert / price-volatility-series），60s 轮询同一把钥匙，盘中按 TTL 刷新、**休市（盘后/午休/周末）冻结只读上一笔**，后端启动时补一次首屏；CTP 账户只留在「我的持仓」 |
