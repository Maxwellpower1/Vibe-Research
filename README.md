...
- **统一报价中心**：`GET /api/market/quotes?codes=` 腾讯批量（5 秒缓存）。驾驶舱指数/榜单/自选/产业链共用同一快照；后端慢或挂了时浏览器直连 `qt.gtimg.cn` / `ifzq` 兜底。板块热点可 10 秒轮播右侧成分股。`em_get` 对 `fflow/kline` 单独 200ms 间隔，蝴蝶图冷启动更快。
