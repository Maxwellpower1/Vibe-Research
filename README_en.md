...
| Page | What's in it |
|---|---|
| 🇨🇳&nbsp;**A-share** | Header: **Review** / **K-line** / **Detail** / **Filings**. Site-wide ticker tape under the header (world indices + gold/oil/BTC, shares the 5s quote hub). Review is a one-screen cockpit: **world indices** (CN/HK/US/FX) / **live sector boards** (leaders left / laggards right; click a board to open constituents on the other half) / sentiment + northbound / **intraday board-flow** (click to filter) / **main-force inflow rank** / **stock ranks with turnover** / **commodities** (futures + **Sunsirs spot/basis**) / limit pools / watchlist (in-panel search, star-to-watch, industry · concepts) / LHB · funds · 8 industry chains (related boards + iwencai refresh). **7×24 news lives in the review news cell** (CLS / Sina / Wallstreetcn) |
...
| 🗄️&nbsp;**Data** | More menu `/data`: local calendar, daily-bar coverage, experiments. Can fill the last 2y of closed bars for the A-share universe |
