...
K 线页（`/a-share?tab=kline`）左自选表（代码/名称/价量涨跌/买卖/换手量比振幅/开高低昨/涨跌停/市值估值，点列头排序）右上分时（右上角进详情/公告）右下日K，点行出图。分时照期权卡：轴铺满 09:30-15:00 午休空着，量柱按当根相对上一分钟红绿（腾讯分时 O/H/L/C 同价, 不当开收）；A 股分时下窗画成交额(标「成交额」, 腾讯累计拆成分钟增量), 日K 下窗画成交量；期权日K/分时拆量窗, 分时持仓黄线叠在量窗（独立轴 `oi`）。加自选与复盘自选格同一套 `GET /api/fin/suggest`（名称/拼音/代码）。A 股顶栏只留复盘/K线。
